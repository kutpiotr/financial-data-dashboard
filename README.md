...
- **Dashboard Power BI** (`dashboard.pbix`) z dwiema stronami analitycznymi
---
...
├── dashboard.pbix              # Dashboard Power BI (2 strony)
...
```bash
git clone https://github.com/kutpiotr/financial-data-dashboard.git
...
Plik `dashboard.pbix` zawiera interaktywny dashboard połączony z bazą `finance.db` przez ODBC.
 
Strony dashboardu:
1. **ETF Prices** — wykres liniowy cen ETF-ów (CSPX.L, SPY, VWRA.L) + karty KPI (ostatnia cena, zmiana dzienna %, zwrot YTD %) + slicer dat i tickerów
2. **FX Rates** — wykres trendów USD/PLN i EUR/PLN + tabela z odchyleniami od 30-dniowej średniej (alert ⚠️ gdy odchylenie >2%) + karty z aktualnymi kursami
Wymagania do otwarcia: [Power BI Desktop](https://powerbi.microsoft.com/pl-pl/desktop/) + [SQLite ODBC Driver](http://www.ch-werner.de/sqliteodbc/). Przed otwarciem pliku uruchom pipeline żeby wygenerować `finance.db`.
